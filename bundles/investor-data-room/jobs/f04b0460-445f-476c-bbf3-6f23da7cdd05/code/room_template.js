...
<div class="ft">Confidential \u00B7 [Your Company], Inc.</div>
</div>
<script>
...
(function(){
  ...
  function renderD(){
    ...
    var content='';
    if(meta.benchmarks){
      content='<div class="bl"><span class="li"><span class="ld lb"></span>Baseline</span><span class="li"><span class="ld lgp"></span>[Company] Score</span></div>';
...
function renderConnector(room, connector) {
  ...
  var introCards = vcKeys.map(k => {
    ...
    var pplH = g.ppl.map(p => {
      ...
      var subj = 'Intro Request: [Company] \u2014 [Your Product]';
      ...
      var mailto = 'mailto:?cc=ceo%40example.com%2Ccto%40example.com&subject='+encodeURIComponent(subj)+'&body='+encodeURIComponent(body);
...
</head><body><div style="text-align:center"><h1 style="font-size:28px;margin-bottom:8px">Investor Data Room</h1><p style="color:rgba(255,255,255,.55)">Access requires a valid investor link.</p></div><script>
(function(){
  var demos = ${JSON.stringify(DEMOS)};
  if(!demos||!demos.length) return;
  var dIdx=0,ov=null;
