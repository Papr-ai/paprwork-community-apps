...
const SCORER_JOB_ID = '76c428ec-e5d9-44f2-ac86-4ce9112d476c';
...
async function loadFeed() {
  ...
  try {
    ...
    // Primary: scored tweets with drafts, sorted by recency + score
    // Uses real-time freshness from created_at_iso instead of frozen hours_old
    // Ranking: blend research-backed viral_score_v2 (early-window signal) with scorer's
    // relevance score. v2 dominates because it correctly handles freshness + log-scale engagement.
    const withDrafts = await query<Tweet>(`
      SELECT *, ROUND((julianday('now') - julianday(COALESCE(created_at_iso, created_at))) * 24, 1) as real_hours_old
      FROM tweets
      WHERE status IN ('new','scored') AND IFNULL(draft_reply, '') <> ''
      ORDER BY (COALESCE(viral_score_v2, 0) * 1.0 + COALESCE(score, 0) * 0.3) DESC,
               datetime(scored_at) DESC
      LIMIT 10
    `);
    ...
    if (withDrafts.length > 0) {
      ...
      // Fallback 1: any scored tweets — same blended ranking
      ...
      if (scored.length > 0) {
        ...
        // Fallback 2: pure viral_score_v2 ranking (no scorer output yet)
        tweets = await query<Tweet>(`
          SELECT *, ROUND((julianday('now') - julianday(COALESCE(created_at_iso, created_at))) * 24, 1) as real_hours_old
          FROM tweets
          WHERE status IN ('new','scored')
          ORDER BY COALESCE(viral_score_v2, 0) DESC,
                   (like_count + retweet_count * 3 + reply_count * 2) DESC
          LIMIT 10
        `);
      }
    }
