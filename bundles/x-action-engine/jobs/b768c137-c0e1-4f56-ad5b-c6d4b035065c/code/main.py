...
def build_prompt(tweets, papr_context, style_examples, feedback_examples):
    ...
    return f"""
You are choosing the best 10 tweets for @shawkatkabbara to engage with right now and writing the actual drafts.

Primary goal:
- grow reach by winning early reply slots on high-signal tweets
- sound exactly like Shawkat: sharp, first-principles, mechanism-driven
...
Feedback loop from Shawkat's own edits. Highest priority: learn from the AFTER versions, not the BEFORE versions:
...
- prefer tweets where Shawkat can add a mechanism, distinction, prediction, or useful reframing
- avoid generic praise, paraphrasing the tweet, and filler
- SKIP any tweet about politics, elections, government policy, culture wars, or partisan topics — score 0 and exclude
- focus on work: AI, dev tools, startups, building, memory/context layer, knowledge graphs
- if a tweet is a benchmark/result post, explain what the result means or what constraint it reveals
- if edit feedback shows Shawkat tends to tighten, sharpen, or add mechanism, do that
...
def call_groq(prompt: str, api_key: str):
    """Call Groq (llama-3.3-70b) — fast and reliable."""
    if not api_key:
...
def heuristic_reply(tweet_text: str, score_type: str):
    ...
    if 'benchmark' in low or 'agi' in low or '%' in text:
        return (
            ...
            'Strong benchmark thread where Shawkat can interpret the result instead of paraphrasing it.'
        )
    if 'multi-agent' in low or 'coordination' in low or 'marl' in low or 'reinforcement' in low:
        return (
            ...
            'Direct match — tweet is about agent coordination/multi-agent systems, Shawkat has first-hand MARL work to reference.'
        )
    if 'agent' in low and ('github' in low or 'deploy' in low or 'ship' in low or 'ci' in low or 'devops' in low or 'rsync' in low or 'cloud' in low):
        return (
            ...
            'Tweet is about agents replacing devops; Shawkat can add the coordination/decision quality angle from MARL work.'
        )
...
def main():
    ...
    # Political blocklist — filter out culture-war / political content
    POLITICAL_BLOCKLIST = [
        'trump', 'biden', 'maga', 'democrat', 'republican', 'gop',
        'congress', 'senate', 'election', 'vote', 'immigration',
        'abortion', 'gun control', 'woke', 'culture war', 'political',
        'left wing', 'right wing', 'marxist', 'fascist', 'socialism',
        'impeach', 'indictment', 'executive order', 'partisan',
        'geopolitics', 'sanctions', 'tariff', 'trade war',
    ]
    political_filter = ' AND '.join(
        f"LOWER(text) NOT LIKE '%{kw}%'" for kw in POLITICAL_BLOCKLIST
    )
    rows = conn.execute(f"""
        SELECT *, (
          velocity_score * 0.45 +
        ...
        FROM tweets
        WHERE status='new' AND {political_filter}
    ...
    results = None
    # Try Groq first (fast, working key), then Gemini, then Anthropic, then OpenAI
    if args.groq_key:
        try:
            results = call_groq(prompt, args.groq_key)
            log.info('Scoring/drafting succeeded via Groq llama-3.3-70b')
        except Exception as e:
            log.warning(f'Groq failed: {e}')
    if results is None and args.google_key:
        try:
            results = call_gemini(prompt, args.google_key)
            log.info('Scoring/drafting succeeded via Gemini 2.5 Flash')
        except Exception as e:
            log.warning(f'Gemini fallback failed: {e}')
    best_anthropic_key = args.anthropic_direct_key or args.anthropic_key
    if results is None and best_anthropic_key:
        try:
