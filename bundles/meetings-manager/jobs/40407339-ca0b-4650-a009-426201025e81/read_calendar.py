...
def main():
    ...
    end_range = start_of_today + datetime.timedelta(days=8)

    start_ns = NSDate.dateWithTimeIntervalSince1970_(start_of_today.timestamp())
    ...
    today_str = start_of_today.strftime('%Y-%m-%dT00:00')
    conn.execute("DELETE FROM calendar_events WHERE start_time < ?", (today_str,))
    ...
    conn.close()
    print(f"\nDone. {inserted} events, {skipped_allday} all-day skipped. {total} total in DB.")
