...
const DB_SOURCE = 'Calendar Reader (40407339)';
...
// Recording elapsed-time display clock (UI only — never queries the DB)
let recTimerToken = 0;
function startRecTimer(): ReturnType<typeof setInterval> {
    ...
    return `
      <div class="prep-doc">
        ...
        <div class="prep-card prep-questions-card">
          <div class="prep-card-label">${icon('chat',13)} Top 3 Questions to Ask ${prep.meetingType === 'vc' ? '<span class="prep-vc-badge">VC · Rubric-scored</span>' : ''}</div>
          <div class="prep-questions-list">
            ${prep.questionsToAsk.map((qq: any, i: number) => `
...
function attachListeners(): void {
  document.getElementById('btn-copy-tab')?.addEventListener('click', () => { void copyActiveTab(); });
  // Tab switching
...
(async () => {
  ...
  // Live updates: refresh whenever any pipeline job writes to the database
  subscribeJobEvents({
    jobIds: [CALENDAR_JOB, RECORDER_JOB, STOP_JOB, WHISPER_JOB, SUMMARIZER_JOB, PREP_JOB, BG_JOB],
    onDbChanged: () => { loadAll().catch(() => {}); },
    onStatusChanged: () => { loadAll().catch(() => {}); }
  });
})();
